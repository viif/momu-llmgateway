...
2. **语义路由**：若未指定 Provider，则进入智能分发。调用网关内置的轻量级本地 Embedding 模型（ONNX）将用户最新指令向量化，与内存中预定义的 " 任务类别原型向量 " 计算余弦相似度。
	- 命中：相似度超过安全阈值（如 0.75），路由至该类别绑定的最优模型组。
	- 拒识：低于阈值则判定为 " 未知意图 "，自动降级并传递至下一策略，防止路由幻觉。
3. **基于能力的路由**：根据 `task_type` 标签选择模型组。
...
2. **缓存查询**: 请求到来时通过本地嵌入引擎（详见 [4.5 本地嵌入引擎](#45-本地嵌入引擎-local-embedding-engine)）生成 Embedding，在缓存中查找相似向量
   - Redis 存储向量 + 余弦相似度搜索
   - 相似度超过阈值（默认 0.95）时命中
   - 按 model 分区缓存
